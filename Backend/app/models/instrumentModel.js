  ...
  getAll: async () => {
    try {
      ...
      SELECT 
        ...
        t.id as teacherId,
        CASE 
          WHEN k2.id IS NOT NULL AND k2.statusz = 'aktiv' THEN 'rented'
          WHEN l.elerhetoseg = 0 THEN 'maintenance'
          ELSE 'available'
        END as status
      FROM hangszerek h
      ...
      LEFT JOIN kolcsonzesek k2 ON h.id = k2.hangszerId AND k2.statusz = 'aktiv'
      GROUP BY h.id
      ORDER BY h.id
    `);
