...
const Admin = () => {
  ...
  const renderTableRow = (item) => {
    ...
    switch (activeTab) {
      ...
      case 'instruments': return (<><td>{item.name}</td><td>{item.category}</td><td>{item.teacher || '-'}</td><td>{item.rentalPrice}</td><td><span className={`status-badge ${item.status === 'available' ? 'status-success' : item.status === 'rented' ? 'status-warning' : 'status-danger'}`}>{item.status === 'available' ? 'Elérhető' : item.status === 'rented' ? 'Kölcsönözve' : 'Szervízben'}</span></td></>);
